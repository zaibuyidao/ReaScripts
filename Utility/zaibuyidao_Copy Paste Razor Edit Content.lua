...
local paste_options = {
  ...
  content = load_bool("paste_content", true),
...
local function save_settings()
  ...
  save_bool("paste_content", paste_options.content)
...
local function loop()
  ...
  if visible then
    draw_section(T.copy, T.copy_button, { 0x2D8C7EFF, 0x36A997FF, 0x24766BFF }, copy_options, copy_razor_edit)
    reaper.ImGui_Separator(ctx)
    draw_section(T.paste, T.paste_button, { 0xC9772BFF, 0xD98B42FF, 0xAA6322FF }, paste_options, paste_razor_edit)
    reaper.ImGui_Separator(ctx)
    push_font(font_small, font_small_size)

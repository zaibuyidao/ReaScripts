...
function main()
    ...
    user_ok, input_cav = reaper.GetUserInputs("添加MIDI硬件输出和接收到选定的轨道", 4, "MIDI硬件输出,MIDI通道序数,接收轨道编号,清除发送接收(0:保留 1:清除)", output_device ..','.. ordinal ..','.. track_num ..','.. toggle)
    ...
    if toggle == "0" then
        ...
        commandID = reaper.NamedCommandLookup("_S&M_SENDS5") -- SWS/S&M: Remove receives from selected tracks

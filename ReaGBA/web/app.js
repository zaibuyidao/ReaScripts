...
window.onNativeState=function(s){const changed=state.game?.hash!==s.game?.hash;state=s;if(s.game?.path)settings.last_rom_directory=parentDirectory(s.game.path);if(s.app_version){$('about-version').textContent=s.app_version;$('footer-version').textContent='v'+s.app_version;}document.body.classList.toggle('playing',s.loaded);scheduleViewport();$('now-title').textContent=s.game?.title||selected?.title||'选择一个游戏';$('now-subtitle').textContent=s.loaded?`${s.game.code} · 240 × 160 · ${s.core}`:'240 × 160 · Game Boy Advance';$('play-status').textContent=s.loaded?(s.running?'正在游玩':'已暂停'):'准备就绪';$('play-dot').className='dot'+(s.running?'':' idle');$('fps').textContent=s.running?`${s.fps.toFixed(1)} FPS`:'— FPS';$('speed').textContent=s.speed+'×';$('toggle').textContent=s.running?'❚❚ 暂停游戏':s.loaded?'▶ 继续游戏':'▶ 开始游戏';$('toggle').disabled=!s.loaded&&!selected;for(const id of ['reset','shot','stop','save'])$(id).disabled=!s.loaded;$('load').disabled=!s.loaded||!slots.find(x=>x.slot===slot)?.exists;if(changed)run(refreshSlots)();if(s.error)toast(s.error,true);};
...
const defaultKeys=['J','K','Space','Return','D','A','W','S','Q','O'];const labels=['A','B','Select','Start','→','←','↑','↓','R','L','加速（按住）'];
function showKeyHint(){const keys=settings.keys||defaultKeys;toast(`${keys[6]} / ${keys[7]} / ${keys[5]} / ${keys[4]} = ↑ / ↓ / ← / → · ${keys[0]} / ${keys[1]} = A / B · ${keys[8]} / ${keys[9]} = R / L · ${keys[3]} = Start · ${keys[2]} = Select · 按住 ${settings.fast_forward_key||'L'} 加速`);}
function renderKeys(){$('keys').replaceChildren();[...(settings.keys||defaultKeys),settings.fast_forward_key||'L'].forEach((key,i)=>{const row=make('div','key-pair');const b=make('button','',binding===i?'请按键…':key);b.onclick=()=>{binding=i;renderKeys();};row.append(make('span','',labels[i]),b);$('keys').append(row);});}
...
$('open').onclick=run(async()=>{const s=await call('open_rom',{initial_path:settings.last_rom_directory||settings.rom_directory||''});if(s){onNativeState(s);await scan();await refreshSlots();showKeyHint();}});
...
$('reset-keys').onclick=run(async()=>{binding=-1;await saveSettings({keys:defaultKeys,fast_forward_key:'L'});renderKeys();toast('已恢复默认键位');});
...
renderSlots();run(async()=>{settings=await call('get_settings');libraryPreference=typeof settings.library_expanded==='boolean'?settings.library_expanded:null;renderLibrarySettings();scheduleViewport();$('rom-directory').value=settings.rom_directory||'';$('integer').checked=settings.integer_scaling!==false;$('vsync').checked=settings.vsync!==false;$('filter').value=settings.filter||'nearest';renderShader();$('bios').value=settings.bios||'';renderKeys();await scan();const s=await call('get_emulator_state');$('volume').value=Math.round(s.volume*100);$('volume-value').textContent=$('volume').value+'%';onNativeState(s);})();// The game framebuffer and PCM never enter this script.

...
--     reaper.SetExtState(key1, key2, serpent.dump(data), false)
-- end
...
--     local func = assert((loadstring or load)(serializedData))
--     return func()
-- end
...
--     for i = 0, reaper.CountTracks(0) - 1 do
--         local track = reaper.GetTrack(0, i)
--         soloTracks[#soloTracks + 1] = {GUID = reaper.GetTrackGUID(track), solo = reaper.GetMediaTrackInfo_Value(track, "I_SOLO")}
--     end
--     saveData("InstantSoloNotefromMousePosition", "SoloTrackRestores", soloTracks)
-- end

-- -- 恢复 solo 状态的 tracks
-- function RestoreSoloTracks()
--     local soloTracks = getSavedData("InstantSoloNotefromMousePosition", "SoloTrackRestores")
--     if not soloTracks then return end
--     local trackLookup = buildTrackLookup()  -- 构建查找表
--     for _, trackData in ipairs(soloTracks) do
--         local track = trackLookup[trackData.GUID]  -- 使用查找表快速定位 track
--         if track then
--             reaper.SetMediaTrackInfo_Value(track, "I_SOLO", trackData.solo)
--         end
--     end
-- end

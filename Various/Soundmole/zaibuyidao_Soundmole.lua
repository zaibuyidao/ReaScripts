...
PEAKTREE_TAB_AUTO_SHRINK     = true  -- 在调整左侧面板大小时，保持所有 PeakTree 标签页可见
...
max_img_h_offset = 300                 -- 当前窗口高度允许的最大偏移
...
function DrawRowPopup(ctx, i, info, collect_mode)
  ...
  if is_item_mode then
    ...
    if reaper.ImGui_MenuItem(ctx, T("Set as Active RS5K Sample"), "Shift+Q") then
      local tr = reaper.GetSelectedTrack(0, 0) or reaper.GetLastTouchedTrack()
  ...
  reaper.ImGui_Separator(ctx)
  -- 添加到 Collections
  if reaper.ImGui_BeginMenu(ctx, T("Add to Collections")) then
...
function RenderPreviewRouteSettingsUI(ctx)
  ...
  reaper.ImGui_SeparatorText(ctx, T("Hardware channels"))
  ...
  -- 经轨道路由
  reaper.ImGui_SeparatorText(ctx, T("Through track routing"))
  reaper.ImGui_BeginDisabled(ctx, not preview_out_to_track)
  reaper.ImGui_Text(ctx, T("Routing mode:"))
  local modes = { T("Auto: first selected, else named"), T("Named track only"), T("First selected track only") }
...
function loop()
  ...
  if visible then
    ...
    local combo_label = T("Select Fields")
    ...
    if reaper.ImGui_BeginCombo(ctx, "##search_fields", combo_label, reaper.ImGui_WindowFlags_NoScrollbar()) then -- reaper.ImGui_ComboFlags_NoArrowButton()
      -- 绘制 All 选项
      local changed_all, new_all_state = reaper.ImGui_Checkbox(ctx, T("All"), all_checked)
      if changed_all then
        for _, field in ipairs(search_fields) do
    ...
    if run_delete_db then
      ...
      if not target_dbfile then
        reaper.ShowMessageBox(T("No database selected."), T("Delete Database"), 0)
      else
    ...
    if h_splitter_drag and reaper.ImGui_IsMouseDown(ctx, 0) then
      ...
      img_h_offset = math.max(0, math.min(max_img_h_offset, h_splitter_start_offset - delta))
    end
    ...
    if ALBUM_PANEL_VISIBLE and album_splitter_drag and reaper.ImGui_IsMouseDown(ctx, 0) then
      album_panel_px = album_splitter_start_w - (select(1, reaper.ImGui_GetMousePos(ctx)) - album_splitter_start_mouse_x)
    end
    ...
    if reaper.ImGui_BeginChild(ctx, "##file_table_child", table_w, child_h, 0) then
      ...
      -- | reaper.ImGui_TableFlags_RowBg()       -- 表格背景交替颜色
        ...
        if collect_mode == COLLECT_MODE_ALL_ITEMS then -- Media Items
          ...
          reaper.ImGui_TableSetupColumn(ctx, T("Track"),       reaper.ImGui_TableColumnFlags_WidthFixed(), 100, TableColumns.DATE)
          reaper.ImGui_TableSetupColumn(ctx, T("Position"),    reaper.ImGui_TableColumnFlags_WidthFixed(), 80, TableColumns.GENRE)
          reaper.ImGui_TableSetupColumn(ctx, T("Comment"),     reaper.ImGui_TableColumnFlags_WidthFixed(), 40, TableColumns.COMMENT)
          reaper.ImGui_TableSetupColumn(ctx, T("Description"), reaper.ImGui_TableColumnFlags_WidthFixed(), 200, TableColumns.DESCRIPTION)
          reaper.ImGui_TableSetupColumn(ctx, T("Category"),    reaper.ImGui_TableColumnFlags_WidthFixed(), 80, TableColumns.CATEGORY)
          reaper.ImGui_TableSetupColumn(ctx, T("SubCategory"), reaper.ImGui_TableColumnFlags_WidthFixed(), 80, TableColumns.SUBCATEGORY)
          reaper.ImGui_TableSetupColumn(ctx, T("CatID"),       reaper.ImGui_TableColumnFlags_WidthFixed(), 80, TableColumns.CATID)
          reaper.ImGui_TableSetupColumn(ctx, T("Length"),      reaper.ImGui_TableColumnFlags_WidthFixed(), 60, TableColumns.LENGTH)
        ...
        elseif collect_mode == COLLECT_MODE_FREESOUND then -- Freesound
          ...
          reaper.ImGui_TableSetupColumn(ctx, T("Size"),        reaper.ImGui_TableColumnFlags_WidthFixed(), 55, TableColumns.SIZE)
          reaper.ImGui_TableSetupColumn(ctx, T("Type"),        reaper.ImGui_TableColumnFlags_WidthFixed(), 40, TableColumns.TYPE)
          reaper.ImGui_TableSetupColumn(ctx, T("Date"),        reaper.ImGui_TableColumnFlags_WidthFixed(), 55, TableColumns.DATE)
          reaper.ImGui_TableSetupColumn(ctx, T("Tags"),        reaper.ImGui_TableColumnFlags_WidthFixed(), 55, TableColumns.GENRE)
          reaper.ImGui_TableSetupColumn(ctx, T("License"),     reaper.ImGui_TableColumnFlags_WidthFixed(), 40, TableColumns.COMMENT)
          reaper.ImGui_TableSetupColumn(ctx, T("Description"), reaper.ImGui_TableColumnFlags_WidthFixed(), 200, TableColumns.DESCRIPTION)
          reaper.ImGui_TableSetupColumn(ctx, T("Category"),    reaper.ImGui_TableColumnFlags_WidthFixed(), 80, TableColumns.CATEGORY)
          reaper.ImGui_TableSetupColumn(ctx, T("SubCategory"), reaper.ImGui_TableColumnFlags_WidthFixed(), 80, TableColumns.SUBCATEGORY)
          reaper.ImGui_TableSetupColumn(ctx, T("CatID"),       reaper.ImGui_TableColumnFlags_WidthFixed(), 80, TableColumns.CATID)
          ...
          reaper.ImGui_TableSetupColumn(ctx, T("Type"),        reaper.ImGui_TableColumnFlags_WidthFixed(), 40, TableColumns.TYPE)
          reaper.ImGui_TableSetupColumn(ctx, T("Date"),        reaper.ImGui_TableColumnFlags_WidthFixed(), 55, TableColumns.DATE)
          reaper.ImGui_TableSetupColumn(ctx, T("Genre"),       reaper.ImGui_TableColumnFlags_WidthFixed(), 55, TableColumns.GENRE)
          reaper.ImGui_TableSetupColumn(ctx, T("Comment"),     reaper.ImGui_TableColumnFlags_WidthFixed(), 40, TableColumns.COMMENT)
          reaper.ImGui_TableSetupColumn(ctx, T("Description"), reaper.ImGui_TableColumnFlags_WidthFixed(), 200, TableColumns.DESCRIPTION)
          reaper.ImGui_TableSetupColumn(ctx, T("Category"),    reaper.ImGui_TableColumnFlags_WidthFixed(), 80, TableColumns.CATEGORY)
          reaper.ImGui_TableSetupColumn(ctx, T("SubCategory"), reaper.ImGui_TableColumnFlags_WidthFixed(), 80, TableColumns.SUBCATEGORY)
    ...
    if h_splitter_drag and active then
      ...
      img_h_offset = math.max(0, math.min(max_img_h_offset, new_off)) -- 专辑封面高度限制
    ...
    do
      ...
      if not h_splitter_drag and bottom_layout_start_y and img_h and img_h > 0 and ui_scale and ui_scale > 0 then

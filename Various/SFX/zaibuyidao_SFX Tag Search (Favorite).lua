...
LIP = require('LIP')
CONFIG = require('config-favorite')
...
function parseCSVLine(line, sep, lineNumber)
    ...
    if countSeparator(line, sep) ~= 2 then
		...
		if retval == 1 then
			openUrl(script_path .. "keywords_favorite.csv")
		end
        return false
    end
...
local data = {}
do
	local f = io.open(script_path .. getPathDelimiter() .. "keywords_favorite.csv", "r")
	if not f then -- 创建自定义关键词文件
		local file_path = script_path  .. "keywords_favorite.csv"
...
attack,攻击,Battle
		...
		f = io.open(script_path .. getPathDelimiter() .. "keywords_favorite.csv", "r")
	end
...
function init()
	...
	function onKeyboardGlobal(key)
		...
		elseif key == 26162 then --F2 编辑配置表
			openUrl(script_path .. "lib/config-favorite.lua")

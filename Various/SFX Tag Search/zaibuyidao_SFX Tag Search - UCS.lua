...
local themes = {
	...
	imgui = {
		...
		text_box_label_active = {1, 1, 1, 1}, -- 文本框文字激活
...
function loadCSV(filePath)
	...
	for line in file:lines() do
		if firstLine then
			...
			if line:match("^%w+") then  -- 确保跳过空行或非数据行
				local cols = {}
				for col in line:gmatch("[^,]+") do
					table.insert(cols, col)
				end
				if #cols >= 7 then  -- 确保行数据列数正确
					local entry = {
						CatID = cols[1],
						SubCategory = cols[2],
						Category = cols[3],
						SubCategory_zh = cols[4],
						Category_zh = cols[5],
						SubCategory_tw = cols[6],
						Category_tw = cols[7],
						--CategoryFull = cols[8] or ""  -- 第8列暂存，处理可能的nil值
					}
	...
	function jGui:processKeyboard()
		...
		if self.lastChar > 0 then
			...
			elseif self.lastChar == self.kb.f9 then
				-- 假设当前语言是全局变量 CURRENT_LANGUAGE，初始化默认为英文 "EN"

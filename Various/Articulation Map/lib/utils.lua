...
function process_and_save_bank_number_mapping(reabank_path)
    local delimiter = package.config:sub(1,1)  -- 获取系统路径分隔符 ('\' for Windows, '/' for Unix)
    local txt_path = reaper.GetResourcePath() .. delimiter .. "Data" .. delimiter .. "zaibuyidao_articulation_map" .. delimiter .. "bank-map.txt"

    -- 读取原始 bank-map123.txt 文件内容到表
    local original_content = {}
    local f = io.open(txt_path, "r")
    if f then
        for line in f:lines() do
            if line:match("^%d+-%d+%s*=%s*%d+-%d+$") then
    ...
    for line in file:lines() do
        local key, values = line:match("^//!%s*bank%s+(%d+-%d+)%s*=%s*(%d+-%d+)%s*$")
        if key and values then
            values = values:gsub("%s+", "")  -- 移除数字间的空格
            table.insert(updates, key .. "=" .. values)
        end
    end
    file:close()
    ...
    for _, line in ipairs(updates) do
        local key = line:match("^(%d+-%d+)=")
        if key and not merged_keys[key] then
            table.insert(merged_content, line)
    ...
    for _, line in ipairs(merged_content) do
        f:write(line .. "\n")
    end

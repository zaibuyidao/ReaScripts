--[[
 * ReaScript Name: Insert CC Curve (Triangle)
 * Version: 1.0
 * Author: zaibuyidao
...
local bottom = reaper.GetExtState("InsterCCCurveTriangle", "Bottom")
if (bottom == "") then bottom = "70" end
local top = reaper.GetExtState("InsterCCCurveTriangle", "Top")
if (top == "") then top = "100" end
local times = reaper.GetExtState("InsterCCCurveTriangle", "Times") 
if (times == "") then times = "16" end
local length = reaper.GetExtState("InsterCCCurveTriangle", "Length")
if (length == "") then length = "240" end
local num = reaper.GetExtState("InsterCCCurveTriangle", "Num")
if (num == "") then num = "12" end
local ccnum = reaper.GetExtState("InsterCCCurveTriangle", "CCNum")
if (ccnum == "") then ccnum = "11" end

local user_ok, user_input_CSV = reaper.GetUserInputs("Insert CC Curve (三角波)", 6, "Starting value 起始點,Highest value 最高點,Repetitions 重複,Length 長度,Points 點數,CC number 控制器編號", bottom ..','.. top ..','.. times .. "," .. length .. "," .. num .. "," .. ccnum)
...
reaper.SetExtState("InsterCCCurveTriangle", "Bottom", bottom, false)
reaper.SetExtState("InsterCCCurveTriangle", "Top", top, false)
reaper.SetExtState("InsterCCCurveTriangle", "Times", times, false)
reaper.SetExtState("InsterCCCurveTriangle", "Length", length, false)
reaper.SetExtState("InsterCCCurveTriangle", "Num", num, false)
reaper.SetExtState("InsterCCCurveTriangle", "CCNum", ccnum, false)
...
reaper.Undo_EndBlock("Insert CC Curve (Triangle)", -1)

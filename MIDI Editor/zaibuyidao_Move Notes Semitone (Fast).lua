-- @description Move Notes Semitone (Fast)
-- @version 1.0
-- @author zaibuyidao
...
for take, _ in pairs(getAllTakes()) do
  ...
  local semitone = reaper.GetExtState("MoveNotesSemitone", "Semitone")
  if (semitone == "") then semitone = "2" end

  uok, uinput = reaper.GetUserInputs("Move Notes Semitone (Fast)", 1, "UP/Down (±)", semitone)
  if not uok then return reaper.SN_FocusMIDIEditor() end
  semitone = uinput:match("(.*)")
  
  semitone = tonumber(semitone)

  reaper.SetExtState("MoveNotesSemitone", "Semitone", semitone, false)
...
reaper.Undo_EndBlock("Move Notes Semitone (Fast)", -1)
reaper.PreventUIRefresh(-1)

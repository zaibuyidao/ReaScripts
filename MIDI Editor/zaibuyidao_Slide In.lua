...
local mode = reaper.GetExtState("SlideIn", "Mode")
if (mode == "") then mode = "1" end

uok, uinput = reaper.GetUserInputs("Slide In", 3, "Pitch Range 彎音範圍,Bezier 貝塞爾 (-100,100),0=Fret 有品 1=Fretless 無品", pitch..','..bezier..','..mode)
...
reaper.SetExtState("SlideIn", "Mode", mode, false)
...
elseif mode == "0" then
  local average_len = math.floor((loop_end - loop_start)/math.abs(pitch))

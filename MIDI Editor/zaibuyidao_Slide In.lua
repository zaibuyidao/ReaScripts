...
userOK, get_value = reaper.GetUserInputs("Slide In", 1, "Pitch Range", "0")
if not userOK then return end

local average_len = math.floor((loop_len)/math.abs(get_value))
...
if tonumber(get_value) < 0 then
  for i = math.floor(get_value), -1, 1 do
...
reaper.Undo_EndBlock("Slide In", 0)

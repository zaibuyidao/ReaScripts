--[[
 * ReaScript Name: Trim Note Right Edge -10
...
for _, v in pairs(pitchNotes) do -- 4.遍歷按音高分組後的音符
    ...
    for i = 1, #v do -- 6.處理音符結束位置
        ...
        if (i == #v) then
            v[i].endPos = v[i].endPos - 10
        else
            v[i].endPos = min(v[i].endPos - 10, v[i + 1].startPos)
        end
        ::continue::
...
reaper.Undo_EndBlock("Trim Note Right Edge -10", -1)

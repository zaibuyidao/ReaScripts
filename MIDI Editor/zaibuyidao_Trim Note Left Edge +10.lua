...
for _, v in pairs(pitchNotes) do -- 4.遍歷按音高分組後的音符
    ...
    for i = 1, #v do -- 6.處理音符結束位置
        ...
        if (10 - length) >= 0 then -- 限制音符長度最小值不再執行操作
            goto continue
        end
        if (i == 1) then
            v[i].startPos = v[i].startPos + 10
        else
            v[i].startPos = max(v[i].startPos + 10, v[i - 1].endPos)
        end
        ::continue::
...
reaper.Undo_EndBlock("Trim Note Left Edge +10", -1)

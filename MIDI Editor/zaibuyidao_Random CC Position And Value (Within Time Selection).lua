-- @description Random CC Position And Value (Within Time Selection)
-- @version 2.3.1
-- @author zaibuyidao
...
elseif language == "繁体中文" then
    ...
    title = "Random CC Position And Value"
end
...
function Main(gridEnabled)
    ...
    for i, idx in ipairs(index) do
        ...
        if retry_count <= max_retries then
            reaper.MIDI_SetCC(take, idx, sel, muted, rand_pos, chanmsg, chan, msg2, tonumber(math.random(diff)), false)
        end
    end

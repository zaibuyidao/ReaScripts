--[[
 * ReaScript Name: Delete Channel Events
 * Version: 1.0
 * Author: zaibuyidao
...
function SelChanEvent(amount)
  ...
  if isSelNote or isSelCC then
    while i ~= -1 do
      ...
      if not (note[i].chan == amount)  then
    ...
    for i = 0, notecnt-1 do
      ...
      if not isSelNote and (chan == amount) then
    ...
    for j = 0, ccevtcnt-1 do
      ...
      if not isSelCC and (chan == amount) then
...
function Main()
  ...
  amount = reaper.GetExtState("DeleteChannelEvents", "Channel")
  if (amount == "") then amount = "1" end
  user_ok, amount = reaper.GetUserInputs("Delete Channel Events", 1, "Channel number", amount)
  amount = tonumber(amount)
  reaper.SetExtState("DeleteChannelEvents", "Channel", amount, false)
  amount = amount - 1
...
local title = "Delete Channel Events"

...
amount = reaper.GetExtState("TrimNoteLeftEdge", "Amount")
if (amount == "") then amount = "10" end
local user_ok, amount = reaper.GetUserInputs("Trim Note Left Edge", 1, "Amount", amount)
if not user_ok then return reaper.SN_FocusMIDIEditor() end
reaper.SetExtState("TrimNoteLeftEdge", "Amount", amount, false)
...
for _, v in pairs(pitchNotes) do -- 4.遍歷按音高分組後的音符
    ...
    for i = 1, #v do -- 6.處理音符結束位置
        local length = v[i].endPos - v[i].startPos
        if not v[i].selected then goto continue end
        if (amount - length) >= 0 then -- 限制音符長度最小值不再執行操作
            goto continue
        end
        if (i == 1) then
            v[i].startPos = v[i].startPos + amount
        else
            v[i].startPos = max(v[i].startPos + amount, v[i - 1].endPos)
        end
        ::continue::
...
reaper.Undo_EndBlock("Trim Note Left Edge", -1)

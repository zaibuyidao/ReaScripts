-- @description Random Note Starts
-- @version 1.0
-- @author zaibuyidao
-- @changelog
--   New Script
...
function main()
    for take, _ in pairs(getTakes) do
        amount = reaper.GetExtState("RandomNoteStarts", "Parameters")
        ...
        elseif language == "繁體中文" then
            ...
            title = "Random Note Starts"
            captions_csv = "Enter A Tick:"
        end
    
        uok, amount = reaper.GetUserInputs(title, 1, captions_csv, amount)
        if not uok or not tonumber(amount) then return reaper.SN_FocusMIDIEditor() end
        amount = tonumber(amount)
        reaper.SetExtState("RandomNoteStarts", "Parameters", amount, false)
        ...
        for i = 0, notecnt - 1 do
            ...
            if note_len > amount then
                if not sel_note then
                    reaper.MIDI_SetNote(take, i, nil, nil, (startppqpos-amount-1)+math.random(amount*2+1), nil, nil, nil, nil, true)
                end
            end
        end

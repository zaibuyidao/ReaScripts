...
user_ok, dialog_ret_vals = reaper.GetUserInputs("Select Wheel", 9, "Wheel,,Tick,,Beat,,Channel,,Enter 1 to restore default settings,", min_val ..','.. max_val ..','.. min_tick ..','.. max_tick ..','.. min_meas ..','.. max_meas ..','.. min_chan ..','.. max_chan ..','.. reset)
if not user_ok then return reaper.SN_FocusMIDIEditor() end
min_val, max_val, min_tick, max_tick, min_meas, max_meas, min_chan, max_chan, reset = dialog_ret_vals:match("(.*),(.*),(.*),(.*),(.*),(.*),(.*),(.*),(.*)")
if not tonumber(min_val) or not tonumber(max_val) or not tonumber(min_tick) or not tonumber(max_tick) or not tonumber(min_meas) or not tonumber(max_meas) or not tonumber(min_chan) or not tonumber(max_chan) or not tonumber(reset) then return reaper.SN_FocusMIDIEditor() end
min_val, max_val, min_tick, max_tick, min_meas, max_meas, min_chan, max_chan, reset = tonumber(min_val), tonumber(max_val), tonumber(min_tick), tonumber(max_tick), tonumber(min_meas), tonumber(max_meas), tonumber(min_chan), tonumber(max_chan), tonumber(reset)

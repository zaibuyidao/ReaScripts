-- @description Move Events Left 10 Ticks (Fast)
-- @version 1.0.1
-- @author zaibuyidao
...
-- @about
--   # Move Events Left 10 Ticks (Fast)
...
for take, _ in pairs(getAllTakes()) do
  ...
  for pitch, pitchNoteEvent in pairs(pitchNoteEvents) do
    for i=1, #pitchNoteEvent do
      ...
      -- 移动音符
      if pitchNoteEvent[i].type == EVENT_NOTE_END then
        if originPos - moveDistance < prePos + moveDistance then overflow = true end
        pitchNoteEvent[i].pos = math.max(prePos + moveDistance + (diff - moveDistance), originPos - moveDistance)
      else

...
function Main()
  ...
  userOK, dialog_ret_vals = reaper.GetUserInputs("Select Note", 2, "Min Key,Max Key", "0,127")
  ...
  if min_val > 127 or max_val > 127 or min_val < 0 or max_val < 0 then return reaper.MB("Please enter a value from 0 through 127", "Error", 0), reaper.SN_FocusMIDIEditor() end

  for i = 0,  notes-1 do
    retval, sel, muted, ppq_start, ppq_end, chan, pitch, vel = reaper.MIDI_GetNote(take, i)
    if sel == true then
      if pitch >= min_val and pitch <= max_val then -- 定义力度范围

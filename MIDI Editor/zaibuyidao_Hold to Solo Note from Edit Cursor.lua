...
local function stash_save_take_events(take)
  ...
  reaper.SetExtState("HoldtoSoloNotefromEditCursor", tostring(take), encodedStr, false)
end

local function stash_apply_take_events(take)
  local base64Str = reaper.GetExtState("HoldtoSoloNotefromEditCursor", tostring(take))
...
function CheckShortcutSetting()
  local shortcutSetting = reaper.GetResourcePath() .. '/Scripts/zaibuyidao Scripts/MIDI Editor/zaibuyidao_Hold to Solo Note Settings.lua'
  ...
  if reaper.file_exists(shortcutSetting) then
    ...
    if reaper.APIExists('ReaPack_BrowsePackages') then
      reaper.ReaPack_BrowsePackages('zaibuyidao Hold to Solo Note Settings')
    else
...
if key == "" then
  ...
  key = reaper.GetExtState("HOLD_TO_SOLO_NOTE_SETTINGS", "VirtualKey")
end

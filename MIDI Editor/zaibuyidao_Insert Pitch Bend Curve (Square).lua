...
get_curve = get_cur_square

function Main()
  local take = reaper.MIDIEditor_GetTake(reaper.MIDIEditor_GetActive())
  if not take or not reaper.TakeIsMIDI(take) then return end

  local bottom = reaper.GetExtState("InsertPitchBendCurveSquare", "Bottom")
  if (bottom == "") then bottom = "0" end
  local top = reaper.GetExtState("InsertPitchBendCurveSquare", "Top")
  if (top == "") then top = "1024" end
  local times = reaper.GetExtState("InsertPitchBendCurveSquare", "Times") 
  if (times == "") then times = "16" end
  local length = reaper.GetExtState("InsertPitchBendCurveSquare", "Length")
  if (length == "") then length = "240" end
  local num = reaper.GetExtState("InsertPitchBendCurveSquare", "Num")
  if (num == "") then num = "2" end

  local user_ok, user_input_CSV = reaper.GetUserInputs("Insert Pitch Bend Curve (Square)", 5, "Starting value 起始點,Highest value 最高點,Repetitions 重複,Length 長度,Points 點數", bottom ..','.. top ..','.. times .. "," .. length .. "," .. num)
  ...
  reaper.SetExtState("InsertPitchBendCurveSquare", "Bottom", bottom, false)
  reaper.SetExtState("InsertPitchBendCurveSquare", "Top", top, false)
  reaper.SetExtState("InsertPitchBendCurveSquare", "Times", times, false)
  reaper.SetExtState("InsertPitchBendCurveSquare", "Length", length, false)
  reaper.SetExtState("InsertPitchBendCurveSquare", "Num", num, false)
  ...
  reaper.Undo_EndBlock("Insert Pitch Bend Curve (Square)", -1)

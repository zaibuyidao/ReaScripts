-- @description Move Notes -01 Semitone (Fast)
-- @version 1.0
-- @author zaibuyidao
...
for take, _ in pairs(getAllTakes()) do
  ...
  for _, event in pairs(noteEvents) do
    ...
    -- 移动音符
    event.pitch = math.max(event.pitch - semitone, 0)
    -- 移动符号
...
reaper.Undo_EndBlock("Move Notes -01 Semitone (Fast)", -1)
reaper.PreventUIRefresh(-1)

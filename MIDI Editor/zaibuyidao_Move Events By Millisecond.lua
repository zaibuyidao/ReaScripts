--[[
 * ReaScript Name: Move Events By Millisecond
 * Version: 1.0
 * Author: zaibuyidao
...
local ms = reaper.GetExtState("MoveEventsByMillisecond", "MS")
if (ms == "") then ms = "100" end
local retval, ms = reaper.GetUserInputs("Move Events By Millisecond", 1, "Enter A Value (ms)", ms)
if not retval then return end
if not tonumber(ms) then return reaper.SN_FocusMIDIEditor() end
reaper.SetExtState("MoveEventsByMillisecond", "MS", ms, false)
...
reaper.Undo_EndBlock("Move Events By Millisecond", -1)

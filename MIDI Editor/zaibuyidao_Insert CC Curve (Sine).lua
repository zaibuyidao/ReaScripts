--[[
 * ReaScript Name: Insert CC Curve (Sine)
 * Version: 1.0
 * Author: zaibuyidao
...
local bottom = reaper.GetExtState("InsterCCCurveSine", "Bottom")
if (bottom == "") then bottom = "70" end
local top = reaper.GetExtState("InsterCCCurveSine", "Top")
if (top == "") then top = "100" end
local times = reaper.GetExtState("InsterCCCurveSine", "Times") 
if (times == "") then times = "16" end
local length = reaper.GetExtState("InsterCCCurveSine", "Length")
if (length == "") then length = "240" end
local num = reaper.GetExtState("InsterCCCurveSine", "Num")
if (num == "") then num = "12" end
local ccnum = reaper.GetExtState("InsterCCCurveSine", "CCNum")
if (ccnum == "") then ccnum = "11" end

local user_ok, user_input_CSV = reaper.GetUserInputs("Insert CC Curve (正弦波)", 6, "Starting value 起始點,Highest value 最高點,Repetitions 重複,Length 長度,Points 點數,CC number 控制器編號", bottom ..','.. top ..','.. times .. "," .. length .. "," .. num .. "," .. ccnum)
...
reaper.SetExtState("InsterCCCurveSine", "Bottom", bottom, false)
reaper.SetExtState("InsterCCCurveSine", "Top", top, false)
reaper.SetExtState("InsterCCCurveSine", "Times", times, false)
reaper.SetExtState("InsterCCCurveSine", "Length", length, false)
reaper.SetExtState("InsterCCCurveSine", "Num", num, false)
reaper.SetExtState("InsterCCCurveSine", "CCNum", ccnum, false)
...
reaper.Undo_EndBlock("Insert CC Curve (Sine)", -1)

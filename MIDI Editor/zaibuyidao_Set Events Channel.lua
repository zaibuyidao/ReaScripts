...
local title, captions_csv, chan_msg, chan_err
if language == "简体中文" then
    title = "设置事件通道"
    captions_csv = "通道编号:"
    chan_msg = "请输入一个介于 1 到 16 之间的值"
    chan_err = "错误"
elseif language == "繁体中文" then
    title = "設置事件通道"
    captions_csv = "通道編號:"
    chan_msg = "請輸入一個介於 1 到 16 之間的值"
    chan_err = "錯誤"
else
    title = "Set Events Channel"
    captions_csv = "Channel Number:"
    chan_msg = "Please enter a value from 1 through 16"
    chan_err = "Error"
end

chan_num = reaper.GetExtState("SET_EVENTS_CHANNEL", "Channel")
if (chan_num == "") then chan_num = "1" end
user_ok, chan_num = reaper.GetUserInputs(title, 1, captions_csv, chan_num)
reaper.SetExtState("SET_EVENTS_CHANNEL", "Channel", chan_num, false)
if not tonumber(chan_num) then return end
chan_num = chan_num - 1
if chan_num < 0 or chan_num > 15 then return reaper.MB(chan_msg, chan_err, 0) end

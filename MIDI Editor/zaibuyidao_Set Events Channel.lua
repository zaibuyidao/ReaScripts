--[[
 * ReaScript Name: Set Events Channel
...
function main()
    ...
    user_ok, chan_num = reaper.GetUserInputs("Set Events Channel", 1, "Channel Number", chan_num)
    reaper.SetExtState("SetEventsChannel", "Channel", chan_num, false)
    if not tonumber(chan_num) then return end
    chan_num = chan_num - 1
    if chan_num < 0 or chan_num > 15 then return reaper.MB("Please enter a value from 1 through 16","Error",0) end
...
main()
reaper.Undo_EndBlock("Set Events Channel", 0)

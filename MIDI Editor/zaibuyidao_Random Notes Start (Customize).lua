--[[
 * ReaScript Name: Random Notes Start (Customize)
 * Version: 1.0
 * Author: zaibuyidao
...
function Main()
    ...
    while i ~= -1 do
        ...
        if note_len > num then
            if note[i].sel then
                reaper.MIDI_SetNote(take, i, nil, nil, (note[i].startppqpos-num-1)+math.random(num*2+1), nil, nil, nil, nil, true)
            end
        end
    ...
    for i = 1, notecnt do
        ...
        if note_len > num then
            if not sel_note then
                reaper.MIDI_SetNote(take, i - 1, nil, nil, (startppqpos-num-1)+math.random(num*2+1), nil, nil, nil, nil, true)
            end
        end
    end
...
local title = "Random Notes Start"

...
key_signature = reaper.GetExtState("HARMONY", "Key")
state_toggle = reaper.GetExtState("HARMONY", "Toggle")
if (key_signature == "") then key_signature = "C" end
if (state_toggle == "") then state_toggle = "0" end
local userOK, userInputsCSV = reaper.GetUserInputs("Generate Chord By Root", 2, "key Signature,0=Default 1=Root 8 Degrees", key_signature..','.. state_toggle)
if not userOK then return reaper.SN_FocusMIDIEditor() end
key_signature, state_toggle = userInputsCSV:match("(%a*),(%d*)")
if not key_signature:match('[%a%.]+') or not state_toggle:match('[%d%.]+') then return reaper.SN_FocusMIDIEditor() end
reaper.SetExtState("HARMONY", "Key", key_signature, false)
reaper.SetExtState("HARMONY", "Toggle", state_toggle, false)
reaper.SetExtState("HARMONY", "ToggleValue", state_toggle, 0)
local has_state = reaper.HasExtState("HARMONY", "ToggleValue")
if has_state == true then
  state = reaper.GetExtState("HARMONY", "ToggleValue")
end
...
reaper.Undo_EndBlock("Generate Chord By Root", 0)

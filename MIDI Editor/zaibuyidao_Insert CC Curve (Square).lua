--[[
 * ReaScript Name: Insert CC Curve (Square)
 * Version: 1.0
 * Author: zaibuyidao
...
local bottom = reaper.GetExtState("InsterCCCurveSquare", "Bottom")
if (bottom == "") then bottom = "70" end
local top = reaper.GetExtState("InsterCCCurveSquare", "Top")
if (top == "") then top = "100" end
local times = reaper.GetExtState("InsterCCCurveSquare", "Times") 
if (times == "") then times = "16" end
local length = reaper.GetExtState("InsterCCCurveSquare", "Length")
if (length == "") then length = "240" end
local num = reaper.GetExtState("InsterCCCurveSquare", "Num")
if (num == "") then num = "2" end
local ccnum = reaper.GetExtState("InsterCCCurveSquare", "CCNum")
if (ccnum == "") then ccnum = "11" end

local user_ok, user_input_CSV = reaper.GetUserInputs("Insert CC Curve (方波)", 6, "Starting value 起始點,Highest value 最高點,Repetitions 重複,Length 長度,Points 點數,CC number 控制器編號", bottom ..','.. top ..','.. times .. "," .. length .. "," .. num .. "," .. ccnum)
...
reaper.SetExtState("InsterCCCurveSquare", "Bottom", bottom, false)
reaper.SetExtState("InsterCCCurveSquare", "Top", top, false)
reaper.SetExtState("InsterCCCurveSquare", "Times", times, false)
reaper.SetExtState("InsterCCCurveSquare", "Length", length, false)
reaper.SetExtState("InsterCCCurveSquare", "Num", num, false)
reaper.SetExtState("InsterCCCurveSquare", "CCNum", ccnum, false)
...
reaper.Undo_EndBlock("Insert CC Curve (Square)", -1)

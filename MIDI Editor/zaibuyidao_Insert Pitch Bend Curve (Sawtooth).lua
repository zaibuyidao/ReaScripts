-- @description Insert Pitch Bend Curve (Sawtooth)
-- @version 1.0
-- @author zaibuyidao
...
function get_cur_straight(bottom, top, num) -- 三角波曲线版本
  local result = {}
...
function get_cur_square(bottom, top, num) -- 方波曲线版本
  local result = {}
  for i = 1, num/2 do
...
function get_cur_sawtooth(bottom, top, num) -- 锯齿波曲线版本
  local result = {}
...
get_curve = get_cur_sawtooth

function Main()
  local take = reaper.MIDIEditor_GetTake(reaper.MIDIEditor_GetActive())
  if not take or not reaper.TakeIsMIDI(take) then return end

  local bottom = reaper.GetExtState("InsertPitchBendCurveSawtooth", "Bottom")
  if (bottom == "") then bottom = "0" end
  local top = reaper.GetExtState("InsertPitchBendCurveSawtooth", "Top")
  if (top == "") then top = "1024" end
  local times = reaper.GetExtState("InsertPitchBendCurveSawtooth", "Times") 
  if (times == "") then times = "16" end
  local length = reaper.GetExtState("InsertPitchBendCurveSawtooth", "Length")
  if (length == "") then length = "240" end
  local num = reaper.GetExtState("InsertPitchBendCurveSawtooth", "Num")
  if (num == "") then num = "12" end

  local user_ok, user_input_CSV = reaper.GetUserInputs("Insert Pitch Bend Curve (Sawtooth)", 5, "Starting value 起始點,Highest value 最高點,Repetitions 重複,Length 長度,Points 點數", bottom ..','.. top ..','.. times .. "," .. length .. "," .. num)
  ...
  reaper.SetExtState("InsertPitchBendCurveSawtooth", "Bottom", bottom, false)
  reaper.SetExtState("InsertPitchBendCurveSawtooth", "Top", top, false)
  reaper.SetExtState("InsertPitchBendCurveSawtooth", "Times", times, false)
  reaper.SetExtState("InsertPitchBendCurveSawtooth", "Length", length, false)
  reaper.SetExtState("InsertPitchBendCurveSawtooth", "Num", num, false)
  ...
  reaper.Undo_EndBlock("Insert Pitch Bend Curve (Sawtooth)", -1)

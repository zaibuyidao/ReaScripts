-- @description Move Events Right 10 Ticks (Fast)
-- @version 1.0.1
-- @author zaibuyidao
...
-- @about
--   # Move Events Right 10 Ticks (Fast)
...
for take, _ in pairs(getAllTakes()) do
  ...
  local events = getAllEvents(take, function (event)
    ...
    elseif event.isCCBZ then
      ccbzMap[event.pos] = event
    end
    ...
    elseif event.selected then
      table.insert(otherSelectedEvents, event) 
    end
  ...
  for pitch, pitchNoteEvent in pairs(pitchNoteEvents) do
    for i=#pitchNoteEvent, 1, -1 do
      ...
      if pitchNoteEvent[i].type == EVENT_NOTE_START then
        ...
        pitchNoteEvent[i].pos = math.min(nextPos - moveDistance - (120 - moveDistance), originPos + moveDistance)
      else
        if originPos + moveDistance > nextPos then overflow = true end
        pitchNoteEvent[i].pos = math.min(nextPos, originPos + moveDistance)
      end
  ...
  -- 移动其他事件
  for _, event in pairs(otherSelectedEvents) do
    event.pos = math.max(0, event.pos + moveDistance)
    if event.type == EVENT_CC and ccbzMap[event.pos] then
      ccbzMap[event.pos].pos = event.pos
    end
  end
...
reaper.Undo_EndBlock("Move Events Right 10 Ticks (Fast)", -1)
reaper.PreventUIRefresh(-1)

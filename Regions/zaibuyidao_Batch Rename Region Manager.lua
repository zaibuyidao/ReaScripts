  ...
  for ii = -step, step do
    for jj = -step, step do
      table.insert(regions, {
        ...
        right = reserver3(r.right + jj * time)
      })
      -- print(tostring(reserver3(r.left + ii * time)) .. " " .. tostring(reserver3(r.right + jj * time)))
    end
  end
end
...
local ok, retvals_csv = reaper.GetUserInputs("Batch Reanme Region Manager", 8, "Rename 重命名,From beginning 截取開頭,From end 截取結尾,At position 位置,To insert 插入,Remove 移除,Find what 查找,Replace with 替換,extrawidth=200", pattern ..','.. begin_str .. ','.. end_str ..','.. position ..','.. insert ..','.. delete ..','.. find ..','.. replace)

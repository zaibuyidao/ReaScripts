...
(() => {
  ...
  const transport = window.chrome?.webview ? data => window.chrome.webview.postMessage(data)
    : window.webkit?.messageHandlers?.sendplus ? data => window.webkit.messageHandlers.sendplus.postMessage(data)
    : null;
  ...
  window.chrome?.webview?.addEventListener("message", event => window.SendPlusReceive(event.data));

  const orderedTracks = () => [...tracks.values()].sort((a,b) => a.index - b.index);
  const isSC = SendPlusRouting.isSC;
  function kind(track) {
  ...
  function visibleTracks() {
    ...
    return SendPlusRouting.visibleHierarchy(list.filter(t => routingInfo.get(t.id)?.sendCount || routingInfo.get(t.id)?.receiveCount), routingInfo, collapsedFolders, false);
  }
  ...
  function refreshAllCanvas() {
    ...
    allCanvasChain = SendPlusRouting.focusedChain(tracks, routingInfo, allCanvasTarget);
  ...
  function foldTracks(list) {
    return SendPlusRouting.visibleHierarchy(list, routingInfo, collapsedFolders, !!$("search").value.trim());
  }
  ...
  function applyState(message) {
    ...
    routingInfo = SendPlusRouting.analyze(tracks, sends);
  ...
  function eligibleDrop(sources, destination) {
    ...
    return !!sources.length && !!displayTrack(destination) && sources.every(id => tracks.has(id) && SendPlusRouting.canAddSource(all, routingInfo, id, destination));
  }

-- @description Hold to Solo Track Without Moving Edit Cursor (Exclude Item Selection)
-- @version 1.0.2
-- @author zaibuyidao
-- @changelog
--   New Script
...
function CheckShortcutSetting()
    local shortcutSetting = reaper.GetResourcePath() .. '/Scripts/zaibuyidao Scripts/Tracks Properties/zaibuyidao_Hold to Solo Track Settings.lua'
    ...
    if reaper.file_exists(shortcutSetting) then
        ...
        if reaper.APIExists('ReaPack_BrowsePackages') then
            reaper.ReaPack_BrowsePackages('zaibuyidao Hold to Solo Track Settings')
        else
...
if key == "" then
    ...
    key = reaper.GetExtState("HOLD_TO_SOLO_TRACK_SETTINGS", "VirtualKey")
end

-- @description Paste Items And Marker In Time Selection (Relative Position)
-- @version 1.0
-- @author zaibuyidao
-- @changelog
--   + New Script
...
local function pasteMarkersAndRegions()
  ...
  for _, marker in ipairs(markers) do
    ...
    if not marker[1] then
      markerId = markerId + 1
      -- 更新marker[5]以使用新的markerId，确保标记ID的唯一性
      marker[5] = markerId
      reaper.AddProjectMarker2(0, false, newStart, 0, name, marker[5], marker[6])
    end
  end
...
main() -- 再粘贴媒体对象
reaper.Undo_EndBlock("Paste Items And Marker In Time Selection (Relative Position)", -1)
reaper.PreventUIRefresh(-1)

...
function expand_ranges(item, keep_ranges, left_pad, right_pad)
  ...
  for i = 1, #keep_ranges do
    ...
    if eq(keep_ranges[i][1], item_pos) or (i > 1 and keep_ranges[i][1] - left_inc < keep_ranges[i - 1][2]) then
      left_inc = 0
    end
    if eq(keep_ranges[i][2], item_pos + item_len) or (i < #keep_ranges and keep_ranges[i][2] + right_inc > keep_ranges[i + 1][1]) then
      right_inc = 0
    end
...
function get_sample_val_and_pos(take, threshold, hysteresis)
  local ret = false
  ...
  local samples_per_channel = math.ceil((accessor_end - accessor_start) * samplerate)
  local sample_index
  local offset
  local samples_per_block = samplerate

  -- print("aa start", accessor_start)
  -- print("aa end", accessor_end)
  -- print("samples_per_channel", samples_per_channel)
  -- print("samples_per_block", samples_per_block)
  -- Find left bound
  sample_index = 0
  offset = accessor_start
  ...
  while sample_index < samples_per_channel do
    ...
    local accessor_ret = reaper.GetAudioAccessorSamples(accessor, samplerate, channels, offset, samples_per_block, buffer)
    if accessor_ret <= 0 then
...
if get == nil then   -- 默认预设
  ...
  set = getMutiInput("Trim Items Edge Settings", 8, "Threshold (dB),Hysteresis (dB),Leading pad (ms),Trailing pad (ms),Fade in (ms),Fade out (ms),Min item length (ms),Snap offset to peak (ms)", threshold_l ..','.. threshold_r ..','.. leading_pad ..','.. trailing_pad ..','.. fade_in ..','.. fade_out ..','.. length_limit ..','.. snap_offset)
...
for _, items in pairs(track_items) do
  for i, item in ipairs(items) do
    ...
    local ret, peak_value_L, peak_pos_L, peak_value_R, peak_pos_R = get_sample_val_and_pos(take, threshold_l, threshold_r)

    if ret and item_len > toms(length_limit) then
      local ranges = { { item_pos + peak_pos_L, item_pos + peak_pos_R } }
      ranges = expand_ranges(item, ranges, toms(leading_pad), toms(trailing_pad))
...
for i = 0, count_sel_items - 1  do
  ...
  if snap_offset > 0 then
    reaper.SetMediaItemInfo_Value(item, 'D_SNAPOFFSET', max_peak_pos(item, snap_offset))

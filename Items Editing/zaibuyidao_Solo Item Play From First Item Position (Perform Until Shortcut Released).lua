--[[
 * ReaScript Name: Solo Item Play From First Item Position (Perform Until Shortcut Released)
 * Version: 1.0.2
 * Author: zaibuyidao
...
--   script_name = "Solo Item Play From First Item Position"
...
key = reaper.GetExtState("SoloItemPlayFromFirstItemPosition", "VirtualKey")
...
function show_select_key_dialog()
    if (not key or not key_map[key]) then
        ...
        key = retvals_csv
        VirtualKeyCode = key_map[key]
        reaper.SetExtState("SoloItemPlayFromFirstItemPosition", "VirtualKey", key, true)
    end
end
